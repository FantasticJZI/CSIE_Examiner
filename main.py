...
# ✨ 使用非同步 Client 避免 Heartbeat Timeout
...
MOTIVATIONAL_QUOTES = [
    ...
    "「最好的寫程序方式就是不寫程序。」— Edsger W. Dijkstra"
]
...
class AnswerModal(ui.Modal, title='📝 提交你的修行答案'):
    answer = ui.TextInput(label='你的回答', style=discord.TextStyle.paragraph, placeholder='戰友，寫下你的邏輯，導師幫你看看...', min_length=5, max_length=500)

    def __init__(self, db, today_q):
        super().__init__()
        self.db = db; self.today_q = today_q

    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.send_message("⏳ AI 導師閱卷中...", ephemeral=True)
        instruction = "你是一位專業的資工所考研名師。針對回答給予具備同理心的建議與詳解。最後一行格式：SCORE_DATA: {\"score\": 1-10, \"is_related\": bool}"
        try:
            # ✨ 非同步呼叫，防止斷線
            ...
            if "SCORE_DATA:" in ai_reply:
                ...
                if data.get('is_related'):
                    ...
                    status = "✨ 獲得經驗！" if not user_info or user_info[1] != datetime.date.today().isoformat() else "💡 今日已領取"
                    if "✨" in status: self.db.add_xp(interaction.user.id, int(10 + data['score'] * 2))
                    embed = discord.Embed(title="🎯 修行結算 (私密)", description=main_text.strip()[:1000], color=0x3498db)
                    embed.add_field(name="狀態", value=status)
                    await interaction.followup.send(embed=embed, ephemeral=True)
                else: await interaction.followup.send("⚠️ 內容不相關。", ephemeral=True)
            else: await interaction.followup.send(ai_reply[:1900], ephemeral=True)
        except Exception as e:
            print(f"批改失敗: {e}"); await interaction.followup.send("🚨 導師大腦暫時離線", ephemeral=True)
...
class ChallengeView(ui.View):
    ...
    @ui.button(label="📝 我要挑戰", style=discord.ButtonStyle.primary, custom_id="submit_answer_csie_v6")
...
class ExaminerCog(commands.Cog):
    ...
    async def push_question(self):
        ...
        try:
            # ✨ 非同步產題
            res = await client.aio.models.generate_content(model=MODEL_NAME, contents=f"產出一題關於 {target} 的資工考研觀念題，50字內。")
            ...
            await channel.create_thread(name=f"【挑戰】{datetime.date.today()}", embed=embed, view=ChallengeView(self.db, q_text))
        except Exception as e: print(f"產題失敗: {e}")

    @commands.command(name="top")
    @is_csie_channel()
    async def top(self, ctx):
        users = self.db.get_top_users(10)
        desc = "".join([f"{'🥇' if i==1 else '🥈' if i==2 else '🥉' if i==3 else f'{i}.'} **{self.bot.get_user(uid).display_name if self.bot.get_user(uid) else uid}** — `{xp} XP` (Lv.{(xp//100)+1})\n" for i, (uid, xp) in enumerate(users, 1)])
        await ctx.send(embed=discord.Embed(title="🏆 考研要塞：首席榜", description=desc or "目前無人上榜", color=0xf1c40f))
...
class TutorCog(commands.Cog):
    def __init__(self, bot):
        self.bot = bot; self.history_cache = {}
    ...
    async def on_message(self, message):
        ...
        if isinstance(message.channel, discord.DMChannel):
            async with message.channel.typing():
                ...
                # ✨ 核心 Prompt 優化：增加排版與專業度限制
                instruction = """你是一位具備『資工教授深度』與『戰友溫感』的考研導師。

                【行為準則】
                1. 暖心開場 (10%)：稱呼戰友，給予情緒共感。
                2. 硬核解惑 (70%)：解釋 OS、計組、資結演算法時，必須給出精確定義與結構化詳解。
                3. 正面解惑：引導後『必須』給出完整解答，不可迴避技術細節。

                【呈現與排版規範】
                - 嚴格禁止長串水平公式（如：1+1=2 這種一整行的數學表達）。
                - 涉及進制轉換、IEEE 754 或複雜運算，強制使用「垂直拆解」並放進「代碼塊 (Code Block)」對齊。
                - 使用 Markdown 表格進行概念對比。
                - 回覆長度必須控制在繁體中文 600 字內，且不用刻意提及你正在進行什麼引導模式。
                - 數學表示式請用純文字或 markdown 呈現，嚴禁使用用戶可能看不懂的複雜 LaTeX 語法。"""

                api_contents = [{"role": e["role"], "parts": [{"text": e["content"]}]} for e in self.history_cache[user_id]]
                api_contents.append({"role": "user", "parts": [{"text": message.content}]})

                try:
                    # ✨ 非同步連線，確保 Heartbeat 不中斷
                    response = await client.aio.models.generate_content(
                        model=MODEL_NAME, contents=api_contents, config={'system_instruction': instruction}
                    )
                    ...
                    if len(self.history_cache[user_id]) > 8: self.history_cache[user_id] = self.history_cache[user_id][-8:]
...
class MyBot(commands.Bot):
    ...
    async def on_ready(self):
        print(f"🚀 {self.user.name} CSIE 非同步穩定版已啟動！")
